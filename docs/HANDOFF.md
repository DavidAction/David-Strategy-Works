...
- Higher-grade submission reports are attached by `build_submission_fidelity_report`, `build_evidence_lock_report`, `build_consultant_review`, `build_ai_cost_ledger`, and `build_secure_transfer_policy`.
- Visual SVG exports are generated by `create_visual_asset_files`.
- AI provider/model readiness is available at `/api/ai/health`; use `/api/ai/health?live=1` only when API keys are configured and a live call is intended.
